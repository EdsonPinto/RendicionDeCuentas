...
def filtrar_df_por_criterios(df, metadata, desde, hasta, ponente, tipo):
...
def obtener_comparativa(
    ...
    df_a = filtrar_df_por_criterios(df, metadata, desde_a, hasta_a, ponente_a, tipo_a)
    df_b = filtrar_df_por_criterios(df, metadata, desde_b, hasta_b, ponente_b, tipo_b)
    ...
    variaciones = {
        "ingresos_pct": pct_change(met_a["ingresos_totales"], met_b["ingresos_totales"]),
        "egresos_pct": pct_change(met_a["finalizados"], met_b["finalizados"]),
        "activos_pct": pct_change(met_a["activos"], met_b["activos"]),
        "sin_salida_pct": pct_change(
            met_a.get("inconsistentes", 0), met_b.get("inconsistentes", 0)
        ),
        "eficiencia_diff": round(met_b["eficiencia"] - met_a["eficiencia"], 2),
    }

    diferencias = {
        "ingresos": met_b["ingresos_totales"] - met_a["ingresos_totales"],
        "egresos": met_b["finalizados"] - met_a["finalizados"],
        "activos": met_b["activos"] - met_a["activos"],
        "sin_salida": met_b.get("inconsistentes", 0) - met_a.get("inconsistentes", 0),
    }
