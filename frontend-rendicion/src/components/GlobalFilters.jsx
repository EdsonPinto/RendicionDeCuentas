...
export function GlobalFilters({
  ...
  hasCambioPonenteSubtype,
  isPeriodoCompActive,
  isMagistradoCompActive,
  isCuelloBotellaActive,
}) {
  ...
  return (
    <>
      <div
        ...
        title={
          isCuelloBotellaActive
            ? "No disponible en Cuello de Botella"
            : undefined
        }
      >
        <div className="date-field">
          <Calendar
            size={14}
            color={datesDisabled ? "#94a3b8" : "#3b82f6"}
            strokeWidth={2.5}
            className="date-icon"
            onClick={() => openPicker("input-desde")}
            style={{ cursor: datesDisabled ? "not-allowed" : "pointer" }}
          />
          <input
            id="input-desde"
            type="date"
            ...
            value={dates.desde}
            onChange={(e) => {
              setDates({ ...dates, desde: e.target.value });
              fetchStats(e.target.value, dates.hasta);
            }}
            style={{ cursor: datesDisabled ? "not-allowed" : "pointer" }}
          />
        </div>
        ...
        <div className="date-field">
          <Calendar
            ...
            strokeWidth={2.5}
            className="date-icon"
            onClick={() => openPicker("input-hasta")}
            style={{ cursor: datesDisabled ? "not-allowed" : "pointer" }}
          />
          <input
            id="input-hasta"
            ...
            value={dates.hasta}
            onChange={(e) => {
              setDates({ ...dates, hasta: e.target.value });
              fetchStats(dates.desde, e.target.value);
            }}
            style={{ cursor: datesDisabled ? "not-allowed" : "pointer" }}
          />
        </div>
      ...
      <select
        ...
        style={{
          ...
          backgroundColor: viewDisabled ? "#f1f5f9" : "#ffffff",
        }}
        title={
          isCuelloBotellaActive
            ? "No disponible en Cuello de Botella"
